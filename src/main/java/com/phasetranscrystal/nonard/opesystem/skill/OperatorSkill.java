...
public class OperatorSkill<O extends OperatorEntity> {
    ...
    //提升到的等级 --> 升级需要的资源
    public final IntFunction<List<IGMaterialExtractor<?>>> extractorsByLevel;
    //技能标签不会直接影响技能的表现(原本有这一设计但是已被移除，我们希望将其转移至技能模板层面)，它只会在ui显示等地方被使用
    protected final HashSet<SkillFlags> skillFlags = new HashSet<>();

    public OperatorSkill(final Skill<O> skill, final int maxLevel, final IntFunction<List<IGMaterialExtractor<?>>> extractorsByLevel) {
        this.skill = skill;
