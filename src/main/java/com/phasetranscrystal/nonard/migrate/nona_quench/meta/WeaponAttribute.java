...
public record WeaponAttribute(ResourceLocation id, double minValue, double maxValue, double basicValue) {
    public static final Codec<WeaponAttribute> CODEC = RecordCodecBuilder.create(i -> i.group(
            ResourceLocation.CODEC.fieldOf("id").forGetter(WeaponAttribute::id),
            Codec.DOUBLE.fieldOf("min").forGetter(WeaponAttribute::minValue),
            Codec.DOUBLE.fieldOf("max").forGetter(WeaponAttribute::maxValue),
            Codec.DOUBLE.fieldOf("basic").forGetter(WeaponAttribute::basicValue)
    ).apply(i, WeaponAttribute::new));
    ...
    public record Modifier(ResourceLocation id, double value, Stage stage) {
        ...
        public static class Manager {
            ...
            public Modifier addModifier(ResourceLocation attributeId, Modifier modifier) {
                if (initialized && true) { //TODO 进行属性存在性校验
            ...
            public void clearAttributeModifiers(ResourceLocation attributeId) {
                ...
                if (removed != null && !removed.isEmpty()) {
                    cachedValues.put(attributeId, getWeaponAttribute(attributeId).basicValue());
                }
            }
            ...
            private void recalculate(@Nullable ResourceLocation attributeId) {
                if (attributeId != null) {
                    cachedValues.put(attributeId, calculateValue(getWeaponAttribute(attributeId)));
                } else {
                    cachedValues.clear();
                    for (WeaponAttribute id : getWeaponAttributes()) {
            ...
            public void initialize(AssembleWeaponType weapon) {
                ...
                attributes.keySet().retainAll(getWeaponAttributes().stream().map(WeaponAttribute::id).toList());
            ...
            // 实际计算逻辑（内部使用）
            private double calculateValue(WeaponAttribute attribute) {
                ...
                double finalPlusSum = 0;

                for (Modifier modifier : modifiers.values()) {
                    switch (modifier.stage()) {
                        case PLUS -> plusSum += modifier.value();
                        case MULTIPLY -> multiplySum += modifier.value();
                        case FINAL_PLUS -> finalPlusSum += modifier.value();
                    }
                }
            ...
            public WeaponAttribute getWeaponAttribute(ResourceLocation id) {
                //TODO
                return null;
            }

            public List<WeaponAttribute> getWeaponAttributes() {
                //TODO
                return List.of();
            }
